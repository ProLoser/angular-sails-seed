    ...
    try {
        ...
        sails.services['token'].getToken(request, function verify(error, token) {
            if (!error && token !== -1) {
                ...
                // If request.user isn't populated by passport, populate through token
                if (!request.user) {
                    sails.models['user']
                        .findOne(request.token.userId)
                        .exec(function exec(error, user) {
                            if (error) throw new Error('Failed to locate user from request token', error);
