...
var AuthController = {
    ...
    register: function(request, response) {
      request.body.protocol = 'local';
      sails.models['user'].register(request.body, function(err, user) {
        if (err) {
          response.json(err.status, {err: err});
          return;
        }
        if (user) {
          response.json({user: user, token: sails.services['token'].issue(user.id)});
        }
      });
    },
        ...
        sails.services['passport'].callback(request, response, function(error, user) {
            request.login(user, function(error) {
                ...
                if (error) {
                    ...
                    response.json({
                        user: user,
                        token: sails.services['token'].issue(user.id)
                    });
                }
            });
        });
